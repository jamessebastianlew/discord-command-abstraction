...
class MessagePattern:
    ...
    def get_arg_typename(p_arg: str):
        ...
        >>> MessagePattern.get_arg_typename('<string:var_name>')
        'string'
        """
        # strip <>
        p_arg = MessagePattern.strip_p_arg(p_arg)

        # default type string
        if not MessagePattern.is_static(p_arg):
            return 'string'
    ...
    def get_arg_type(p_arg: str):
        ...
        """
        typename = MessagePattern.get_arg_typename(p_arg)
    ...
    def translate_var(p_arg: str, arg: str):
        ...
        """
        argtype = MessagePattern.get_arg_type(p_arg)
        return argtype(arg)
    ...
    def get_p_arg_name(p_arg: str):
        p_arg = MessagePattern.strip_p_arg(p_arg)
        if MessagePattern.is_static(p_arg):
    ...
    def _can_translate_args(self, arguments: List[str]):
        ...
        for p_arg, arg in zip(self.pattern, arguments):
            if is_variable_argument(p_arg) and MessagePattern.is_static(p_arg):
                try:
                    MessagePattern.translate_var(p_arg, arg)
                except:
                    return False
    ...
    def gen_object(self, arguments: List[str]):
        ...
        for:
        self.pattern: MessagePattern('list <int:something>').pattern
        arguments: ['list', '1']
        ...
        for p_arg, arg in zip(self.pattern, arguments):
            ...
            p_arg_name = MessagePattern.get_p_arg_name(p_arg)
            arg_val = MessagePattern.translate_var(p_arg, arg)
